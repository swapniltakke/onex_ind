...
$menu_header_display = 'Add Product';
?>
<head>
    <title>OneX | Add Product</title>
...
<body>
<div id="wrapper">
    <?php $activePage = '/add_checklist_product.php'; ?>
                ...
                <div class="sixteen wide column" style="padding: 0;">
                    <div class="ui segment middle aligned" style="padding-right: 1px;">
                        <div class="ui three column grid" style="width: 100.5%;">
                            <div class="column">
                                <div id="insert_data_product" style="display:block;">
                                    <form id="addProductForm" method="post" style="display: flex; align-items: center;">
                                        <label for="productName" style="margin-right: 10px;"><span class="required-field">*</span>Product Name:</label>
                                ...
                                <div id="edit_data_product" style="display:none;">
                                    <form id="editProductForm" method="post" style="display: flex; align-items: center;">
                                        <input type="hidden" id="productIdEdit" name="productIdEdit" value="">
                                        <label for="productName" style="margin-right: 10px;"><span class="required-field">*</span>Product Name:</label>
                                        <div class="ui input fluid" style="width: 290px; margin-right: 10px;">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Added Product Names</h3>
                    </div>
                    <div class="card-body">
    ...
    <?php $footer_display = 'Add Product';
...
<script>
    ...
    function fetchAndDisplayProducts(page = 1) {
        ...
        $.ajax({
            url: '/dpm/dwc/api/checklistAPI.php?type=view_product',
            type: 'POST',
            data: { page: page },
    ...
    // Function to check if a product name already exists
    function isProductNameExists(productName, callback) {
        showFullLoader();
        activeAjaxRequests++;

        $.ajax({
            url: '/dpm/dwc/api/checklistAPI.php?type=check_product_name',
            type: 'POST',
    ...
    $('#addProductForm').submit(function(e) {
        ...
        isProductNameExists(productName, function(exists) {
            if (exists != 'false') {
                alert('Product name already exists. Please enter a unique name.');
            } else {
                showFullLoader();
                activeAjaxRequests++;

                $.ajax({
                    url: '/dpm/dwc/api/checklistAPI.php?type=insert_product',
                    type: 'POST',
                    data: { productName: productName },
                    success: function(response) {
                        ...
                        fetchAndDisplayProducts(); // Refresh the products table
                        $('#productName').val(''); // Clear the input field
                        activeAjaxRequests--;
                        $('#insert_data_product').show();
                        if (activeAjaxRequests === 0) {
                            hideFullLoader();
                        }
                    },
    ...
    // Function to populate the input field with the selected product name
    function editProduct(productId, productName) {
        $('#productIdEdit').val(productId);
        $('#productNameEdit').val(productName);
        $('#insert_data_product').hide();
        $('#edit_data_product').show();
    }
    ...
    $('#editProductForm').submit(function(e) {
        ...
        isProductNameExists(productName, function(exists) {
            if (exists != 'false' && exists.id !== parseInt(productId)) {
                alert('Product name already exists. Please enter a unique name.');
            } else {
                showFullLoader();
                activeAjaxRequests++;
                ...
                $.ajax({
                    url: '/dpm/dwc/api/checklistAPI.php?type=update_product',
                    type: 'POST',
    ...
    // Initially, show the add product form and hide the edit product form
    $('#insert_data_product').show();
    $('#edit_data_product').hide();
</script>
</body>
