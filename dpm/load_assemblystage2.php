...
$query = DbManager::fetchPDOQuery('spectra_db', $sql, [":stage_id" => "5", ":product_id" => "$product_id", ":station_id" => "%$station_id%"])["data"];
//echo $sql;exit;	
...
$cnt = count($query);
	...
	/* if(odbc_result($query,'check_req')== 'Y')
	 {
		 $text_req='<input type="checkbox" value="1" name="actual_opt[]" checked> Yes';
	 }
	 else{
		$text_req='<p style="display:none;"><input type="checkbox" value="0" name="actual_opt[]" onclick="return false" checked> No</p>';
