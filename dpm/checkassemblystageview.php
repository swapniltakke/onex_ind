...
$rs = DbManager::fetchPDOQuery('spectra_db', $sql_station, [":station_id" => "$station_id"])["data"];
$stage_id = $rs[0]['stage_id'];

$sql = "SELECT * FROM tbl_stage where stage_type=:stage_type and stage_id in (:stage_id)";
$query = DbManager::fetchPDOQuery('spectra_db', $sql, [":stage_type" => "Assembly", ":stage_id" => "$stage_id"])["data"];

$sqlworkbench = "select * from tbl_workbench";
$querywork = DbManager::fetchPDOQuery('spectra_db', $sqlworkbench)["data"];

$sql1 = "SELECT * FROM tbl_transactions where user_id=:user_id";
$query1 = DbManager::fetchPDOQuery('spectra_db', $sql1, [":user_id" => "$user_id"])["data"];
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="row">
			...
			<div class="card">
			   <div class="card-body formarea smpadding">
         <div class="row">
         <?php $i=1; foreach ($querywork as $data) { ?>
          <div class="col-md-4"><a style="cursor: pointer;" data-id="<?php echo $data['id']; ?>" work-id="<?php echo $data['title']; ?>" data-toggle="modal" class="workbench_clk" data-target="#myModal"><?php echo $data['title_icon']."<br>".$data['title']; ?>
         </a>
        </div>
         <?php $i++;  } ?>
         </div>
...
if($i == 1){
echo '<div><h7>'.strtoupper($data['stage_name']).'</h7><br><button type="button" id="div_check'.$i.'"  title="" class="btn btn-default btn-circle btn-xl">'.$arr[$j].'
                            </button>
</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
} else 
{
echo '<div><h7>'.strtoupper($data['stage_name']).'</h7><br><button type="button" id="div_check'.$i.'"  title="" class="btn btn-white btn-circle btn-xl">'.$arr[$j].'
                            </button>
</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
}
$j++; $i++; } ?>
...
'language': {
     ...
     "ajax": {
        'type': 'POST',
        'url': 'load_assemblystage1_view.php',
        'data': function (data) {
