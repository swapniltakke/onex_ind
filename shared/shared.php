...
/**
 * returns custom Http response. Check http codes and types in shared/shared.php->getHttpStatusText()
 * @param int $code
 * @param string $message
 * @param callable|null $callback
 */
function returnHttpResponse(int $code, string $message, ?callable $callback = null, $httpErrorMsg = null): void
{
    echo json_encode([
        "code" => $code,
        "data" => [],
    ...
    header($_SERVER["SERVER_PROTOCOL"] . " $code $statusText");
    $message = $httpErrorMsg ?? $message;
...
/**
 * Creates given directory path. At least the root folder must be present. Returns false if fails
 * @param string $code
 */
    ...
    try {
        ...
        while (false !== ($Item = readdir($Resource))) {
            ...
            if ($Recursive === true && is_dir($Item)) {
                ...
                $Found[] = $Directory . DIRECTORY_SEPARATOR . $Item;
            }
        }
    } catch (Exception $e) {
        return [];
...
//Deletes all contents of given $dir
function removeDirRecursive($dir, $removeItself = true){
    $folderContent = scandir($dir);
    if($folderContent === false)
        return true;
    $files = array_diff($folderContent, array('.', '..'));

    foreach ($files as $file)
        (is_dir("$dir/$file")) ? removeDirRecursive("$dir/$file") : unlink("$dir/$file");

    return !($removeItself) || rmdir($dir);
}

function sanitizeFileName($folderName, $safeChar = null) {
    $forbiddenChars = ['<', '>', ':', '"', '/', '\\', '|', '?', '*'];
    $safeChar = $safeChar ?? '_';
    if(in_array($safeChar, $forbiddenChars))
        throw new Error("Character: $safeChar can not be used on filenames");

    return str_replace($forbiddenChars, $safeChar, $folderName);
}

function getCWDays($yearcw)
{
    $year = substr($yearcw, 0, 4);
    $cw = substr($yearcw, -2);
    $week_start = new DateTime();
    $week_start->setISODate($year, $cw);
    $week = array();
    for ($i = 0; $i < 6; $i++) {
        $week[] = $week_start->format('Y-n-j');
        $week_start->modify('+1 day');
        $month = explode('-', $week[$i])[1];
        $day = explode('-', $week[$i])[2];
        ...
        $week2[$i] = explode("-", $week[$i])[0] . $month2[$i] . $day2[$i];
        $weekDays[] = $week2[$i];
    }
    return $weekDays;
}

function getOneXMailContent($bodyContent = "", $headerContent = ""): string
{
    $selfOrgCode = SharedManager::getFromSharedEnv('SELF_ORG_CODE');
    $onexURL = "https://" . $_SERVER['HTTP_HOST'];
 ...
 <body style="padding: 0; margin: 0; font-size: 12px; line-height: 16px; background: #E1E1D7; height: 100%; width: 100%; mso-line-height-rule: exactly; -ms-text-size-adjust: 100%; -webkit-text-size-adjust: 100%; font-family: Arial, Helvetica, sans-serif;" bgcolor="#E1E1D7" text="#000000" marginheight="0" marginwidth="0" leftmargin="0" topmargin="0">
  ...
  <a name="top" style="text-decoration: none; margin: 0px; padding: 0px; mso-line-height-rule: exactly; -ms-text-size-adjust: 100%; -webkit-text-size-adjust: 100%;"></a>
                ...
                <tbody>
                 <tr> 
                  <td class="sim-row-edit" style="margin: 0px; padding: 0px; line-height: 20px; font-family: Arial, Helvetica, sans-serif; font-size: 14px; border-collapse: collapse; -ms-text-size-adjust: 100%; -webkit-text-size-adjust: 100%; mso-line-height-rule: exactly; mso-table-lspace: 0pt; mso-table-rspace: 0pt;" data-type="text2"> <span style="margin: 0px; padding: 0px; font-family: Microsoft JhengHei; -ms-text-size-adjust: 100%; -webkit-text-size-adjust: 100%; mso-line-height-rule: exactly;"><span class="sim-row-edit-span" style="margin: 0px; padding: 0px; color: rgb(45, 55, 60); line-height: 20px; font-family: Arial, Helvetica, sans-serif; font-size: 14px; display: block; -ms-word-wrap: break-word; min-width: 530px; max-width: 530px; -ms-text-size-adjust: 100%; -webkit-text-size-adjust: 100%; overflow-wrap: break-word; mso-line-height-rule: exactly; -webkit-line-break: after-white-space;">
                    Hello,<br><br>
                    <b>OneX</b> is your new Digital Assistant to facilitate your daily work<br><br>
                    ' . $bodyContent . '
                    <br><br>
                    <br>
                    This is an automated email from <b>OneX</b><br>
                    '.$selfOrgCode.'
                    If you think there is an error with this email or process, please contact <b>'.$selfOrgCode.'</b><br><br>
                    Best Regards,<br>
                    <b>'.$selfOrgCode.'</b>
                  </td> 
                 </tr> 
