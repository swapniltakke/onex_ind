    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
       ...
       <select name="break_type" class="select2 form-control" id="break_type">
                            ...
                            <?php 
                            $sql_rework="select w_type,w_id from tbl_manageworkbench where workbench_id='3'";
                            $res=odbc_exec($conn,$sql_rework);
                            while(odbc_fetch_row($res)) { 
         ...
         <!-- Missing Material -->
         <div id="div_miss_mat" style="display:none">

<label>Types of Missing Materials: </label>
<select name="missing_type" class="select2 form-control" id="missing_type">
                 <option value="">Select Material Type</option>

                 <?php 
                 $sql_rework="select w_type,w_id from tbl_manageworkbench where workbench_id='4'";
                 $res=odbc_exec($conn,$sql_rework);
                 while(odbc_fetch_row($res)) { 
       ...
       <input type="text" name="missing_rem" id="missing_rem" class="form-control"> 
       <label>Start Date: </label>
                      <input type="text" class="form-control" name="start_date_missing" id="start_date_missing" value ="<?php echo date('Y-m-d h:i:s'); ?>" readonly>
                      <label>End Date: </label>
...
{
echo '<div><h7>'.strtoupper(odbc_result($query,'stage_name')).'</h7><br><button type="button" id="div_check'.$i.'"  title="" class="btn btn-white btn-circle btn-xl">'.$arr[$j].'
                            </button>
</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
}
$j++; $i++; } ?>
...
'language': {
     ...
     "ajax": {
        'type': 'POST',
        'url': 'load_subassemblystage1.php',
        'data': function (data) {
...
//Date range picker
    ...
    //Date range picker
    $('#start_date_rework,#end_date_rework,#start_date_break,#end_date_break').datetimepicker();
...
$.ajax({
  ...
  var start_date_missing = $('#start_date_missing').val();
  var end_date_missing = $('#end_date_break').val();
  var missing_rem = $('#missing_rem').val();
  if(missing_type == ""){
    alert("Please Select Missing Materials Type");
    return false;
  }
  else {
$.ajax({
           ...
           data: {
              ...
              work_id : work_id,
              stage_id: 1,
              work_type: missing_type,
              start_date: start_date_missing,
              end_date : end_date_missing,
              rework_remark : missing_rem,
              scanqrcode : scanqrcode 
           },
           success: function (response) {
            if(response.trim() == "SAVED")
 ...
 {
   if(work_id == '5'){
    var kitting_remark =$('#alert_remark').val();
   }
   else if(work_id == '7'){
