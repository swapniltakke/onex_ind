...
$menu_header_display = 'Material Search';
?>
<head>
    <title>OneX | Material Search</title>
...
<body>
<div id="wrapper">
    <?php $activePage = '/material_search.php'; ?>
    <?php include_once '../shared/sidebar.php'; ?>
    <div id="page-wrapper" class="gray-bg">
        <?php include_once 'header.php'; ?>
        <br>
        <div id="projectDetailsSection" style="display: none; margin-top: -1em;">
            <div class='ui active tab segment cont' style='position: relative;'>
                <div style='display: flex; justify-content: space-between; align-items: center; margin-top: -64px!important; padding: 31px!important;'>
                    <div class='ui accordion' style='width: 100%; display: flex; align-items: center;'>
                        <div class='title' style='width: 100%;'>
                            <div class='ui sixteen column grid' style='width: 100%;'>
                                <div class='row'>
                                    <!-- PROJECT DETAILS -->
                        ...
                        <div id='sourceTypeSelect' class='ui fluid selection dropdown' style='width: 200px; margin-right: -25px;'>
                            <i class='dropdown icon'></i>
                            <input type='hidden' name='sourceTypeInput'>
                            <div class='default text' data-translate='source-select'>Source Type</div>
                            <div class='menu'>
                ...
                <div class='content'>
                    ...
                    <div class='ui stackable responsive grid'>
                        ...
                        <div class='ui section' id="materialSearchDataParent" style="display: none">
                            <div id='orderinfo'>
                                <table id='orderInfoDataTable' class='ui green unstackable celled striped table display responsive nowrap' cellspacing='0' width='100%' >
                                    <thead>
    ...
    <?php $footer_display = 'Material Search';
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('.ui.search')
            .search({
                ...
                onSelect: function(result, response) {
                    // Additional actions on project selection
                    if (result && result.project_no) {
                        // Immediate POST request without setTimeout
                        $.ajax({
                            url: '/dpm/dwc/set_project_session.php', // Full absolute path
                            method: 'POST',
                            data: { 
                            ...
                            success: function(response) {
                                console.log('Session update success:', response);
                            },
        ...
        if (panelNumber) {
            saveSelectedValuesToServerMaterial("panel");
            // Trigger the panel dropdown change handler
            initOrderInfoDatatable(panelNumber, "", currentDTFilter);
        ...
        (async () => {
            ...
            setTimeout(() => {
                ...
                if (sourceType) {
                    $('#sourceTypeSelect').dropdown('set selected', sourceType);
                }
            }, 100);
            ...
            $('#panelSearch').dropdown('setting', 'onChange', function(){
                ...
                saveSelectedValuesToServerMaterial("panel");
                initOrderInfoDatatable(selectedPanelNumber, sourceType, currentDTFilter);
                initOrderInfoTableFilters();
            });

            // Source type dropdown change handler
            $('#sourceTypeSelect').dropdown('setting', 'onChange', function(){
                let selectedPanelNumber = ($('#panelSearch').dropdown('get value') || '').match(/^[^|]*/)[0];
                let selectedSourceType = $('#sourceTypeSelect').dropdown('get value');
                if(!selectedSourceType) return;
                if(!selectedPanelNumber) return;
                
                initOrderInfoDatatable(selectedPanelNumber, selectedSourceType, currentDTFilter);
                initOrderInfoTableFilters();
            });
    ...
    function initOrderInfoDatatable(panelNumber, sourceType, searchParams = {}){
        ...
        $('#orderInfoDataTable').DataTable({
            ...
            scrollY: '50vh', // Set a max height to 50% of the viewport height
            scrollCollapse: true, // Allow the table to collapse to a smaller height
            paging: true,
            pageLength: 25,
            ajax: `/dpm/dwc/materialsearch/api/materialsearch.php?project=${projectnumber}&panel=${panelNumber}&source_type=${sourceType}${searchQueryString}`,
            columnDefs: [
    ...
    function saveSelectedValuesToServerMaterial(type) {
        let data = {};
