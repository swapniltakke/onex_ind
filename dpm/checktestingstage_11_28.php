    ...
    if($subassem_req == 'Yes')
    {
       ...
       // echo $check_step; exit;
            ...
            $sql = " select count(*) as cnt from tbl_transactions where product_id = '".$product_id."' and barcode='".$scanqrcode."'  and status =1 and station_id = '".$station_id."'  ";
               ...
               if($count_rs < 1)
			   {
				...
				//To update status as Subassembly   
					...
					$check_Uploadid="select top 1 uploadid as count_rs from tbl_DailyUpload where SUBSTRING(Product,0,9) = '".$scanqrcode1."' and progress_status != 4" ;	
            ...
            if($cnt < 1){
               ...
               if($count_rs < 1){
				   ...
				   $check_Uploadid="select top 1 uploadid as count_rs from tbl_DailyUpload where SUBSTRING(Product,0,9) = '".$scanqrcode1."' and progress_status != 4" ;	
					...
					$sql2="update tbl_DailyUpload set Progress_Status = 1, barcode = '".$scanqrcode."' where where uploadid = '".$cnt_uploadid."'" ;	
					//echo $sql2; exit;
					$resuploadid = odbc_exec($conn, $sql2);
					...
					if($cnt_loc2 == 5 || $cnt_Stationstage == '6,7') //Testing
					{
						...
						//{
							...
							else
							{
								...
								{
									$check_DupStation="select count(*) as total,station_id as stid from tbl_station where concat(',', product_id, ',') like '%,$product_id,%' and concat(',', stage_id, ',') like '%5%'  group by station_id";
									$result_loc1 = odbc_exec($conn, $check_DupStation);
									$B2B3station =0;
									$A6A7station =0;
									while($row = odbc_fetch_row($result_loc1)) 
									{ 
										switch (odbc_result($result_loc1,'stid')) 
										{
											case "8":
												if($B2B3station != 0)
												{
        ...
        else
		{
            ...
            $rs_set1 = odbc_exec($conn, $check_step1);
            $cnt_rs1 = odbc_result($rs_set1,'count_rs');
			//echo $cnt_rs1; exit;
			if ($cnt_rs1 == 1)
			{
				...
				//-----------------------------------------------------------------------------------
				$sql1 = " select count(*) as cnt from tbl_transactions where product_id = '".$product_id."' and barcode='".$scanqrcode."'  and stage_id='6' and station_id = '".$station_id."'  ";
                //echo $sql1;exit;
				$resultset1 = odbc_exec($conn, $sql1);
				$count_rs1 = odbc_result($resultset1,'cnt');
				//echo $count_rs1;exit;
				if($cnt_loc1 == $cnt_total_loc1)
				{
					$res['cnt']=0;
