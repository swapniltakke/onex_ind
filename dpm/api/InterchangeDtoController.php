...
class InterchangeDtoController extends BaseController
{
    public function getInterchangeDtosOfNachbau(): void {
        ...
        $query = "SELECT DISTINCT kmat_name as dto_number, description, typical_no
                  ...
                  AND kmat_name LIKE '%::%'                  
                  AND kmat_name REGEXP (SELECT d.rules FROM dto_configurator.rules d WHERE d.key='nachbau_dto_names')
                  ORDER BY dto_number";

        $result = DbManager::fetchPDOQueryData('planning', $query, [':pNo' => $projectNo, ':nNo'=>$nachbauNo, ':accTypical' => $accTypical])['data'] ?? [];
