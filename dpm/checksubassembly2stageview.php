...
$sql = "SELECT * FROM tbl_stage where stage_type='Subassembly' and stage_id in ($stage_id)";
  ...
  <a href="#"  class="tablinks paybill" ><i class="fa fa-money"></i> </a>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <p align="right"> <button class="btn btn-primary btn-sm" id="view_rework" >View Rework&nbsp;<i class='far fa-file-pdf'></i></button></p>
       ...
       <!-- <table id="example2" class="table table-bordered table-striped">
          ...
          <select name="w_type" class="select2 form-control" id="w_type">
                            ...
                            <?php 
                            $sql_rework="select w_type,w_id from tbl_manageworkbench where workbench_id='1'";
                            $res=odbc_exec($conn,$sql_rework);
                            while(odbc_fetch_row($res)) { 
                               
                                    echo '<option value="'.odbc_result($res,'w_id').'">'.odbc_result($res,'w_type').'</option>';
                                    ...
                                    <input type="text" class="form-control" name="start_date_rework" id="start_date_rework">
                                    <label>End Date: </label>
                                    <input type="text" class="form-control" name="end_date_rework" id="end_date_rework">

                              
          </div>

          <div id="work_sop" style="display:none">
         <p align="right"> <button class="btn btn-primary btn-sm" id="view_sop_files" >View SOP Files&nbsp;<i class='far fa-file-pdf'></i></button></p>
          <br>
          <label>PDF UPLOAD: </label>
                 <input type="file" class="form-control" name="file[]" id="sop_file"  multiple="multiple">
              ...
              <div id="div_breakdown" style="display:none">
                     ...
                     <label>Start Date: </label>
                                    <input type="date" class="form-control" name="start_date_break" id="start_date_break">
                                    <label>End Date: </label>
                                    <input type="date" class="form-control" name="end_date_break" id="end_date_break">
        ...
        <input type="button" class="btn btn-warning" value="Save" id="save_workbench" >  <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
        </div>
      </div>
    ...
    <div class="modal-dialog modal-lg">
      ...
      <div class="modal-content">
        ...
        <div class="modal-footer">
            ...
            <input type="hidden" id="checkstage_id" value="2"> 
                ...
                <br>
             <p align="right"><input type="button" class="btn btn-primary" id="check_submit" name="Submit" value="Submit"> 
...
if($i == 1){
echo '<div><h7>'.strtoupper(odbc_result($query,'stage_name')).'</h7><br><button type="button" id="div_check'.$i.'"  title="'.strtoupper(odbc_result($query,'stage_name')).'" stage-id="'.odbc_result($query,'stage_id').'" data-id="'.odbc_result($query,'stage_name').'" data-toggle="modal" data-target="#myModal1" class="btn btn-success btn-circle btn-xl_pre stage_clk">'.$arr[$j].'
                            </button>
</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
}
if($i == 2){
    echo '<div><h7>'.strtoupper(substr(odbc_result($query,'stage_name'),0,14)).'..</h7><br><button type="button" id="div_check'.$i.'"  title="'.strtoupper(odbc_result($query,'stage_name')).'" class="btn btn-default btn-circle btn-xl">'.$arr[$j].'
    </button>
</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
}else if($i != 1 && $i != 2 )
{
echo '<div><h7>'.strtoupper(odbc_result($query,'stage_name')).'</h7><br><button type="button" id="div_check'.$i.'"  title="'.strtoupper(odbc_result($query,'stage_name')).'" class="btn btn-white btn-circle btn-xl">'.$arr[$j].'
                            </button>
</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
}
$j++; $i++; } ?>
...
'language': {
    ...
    "ajax": {
        'type': 'POST',
        'url': 'load_subassemblystage2view.php',
        'data': function (data) {
		...
		data.station_id=<?php echo $station_id;?>;
         ...
         "aoColumns": [
          {mData: 'sr_no',sWidth: '5%'},
             ...
             return '<input type="radio" class="check_req" value="1" name="actual_opt'+row['sr_no']+'" id="check_req'+row['sr_no']+'" > Ok<input type="radio" class="check_req" value="0" name="actual_opt'+row['sr_no']+'" id="check_req'+row['sr_no']+'"> Not Ok';
                      },
                      sWidth: '20%'
                    },
...
if(remarkcnt.length == '') 
{
 ...
 if (confirm('Are you sure you want to complete second stage?')) {
       $.ajax({
           url: 'update_secondstage.php',
           type: "POST",
           data: {
           ...
           var x = data.success 
               if(x.saved == "SAVED" && x.complete == "COMPLETE")
                   ...
                   else if(x.saved == "SAVED" && x.notcomplete == "NOTCOMPLETE")
                   {
                        alert("Data is saved");
...
//Date range picker
    ...
    //Date range picker
    $('#start_date_rework,#end_date_rework').datetimepicker();
...
var station_id = '<?php echo $station_id;?>';
 ...
 if(work_id == '1'){
  ...
  if(work_type == "" && start_date == "" && end_date == ""){
    alert("Please Select Work Type, Start Date and End Date");
    return false;
  }
  else {
//alert('Hello');
