...
where  c3.stage_id='1' and p.product_id='$product_id'
		...
		while(odbc_fetch_row($query))
{
	$response1 ="";
	 ...
	 /*if(odbc_result($query,'check_req')== 'Y')
	 {
		 $text_req='<input type="radio" class="check_req."'.$sr.'"" value="1" name="actual_opt[]" > Ok &nbsp;<input type="radio" class="check_req."'.$sr.'"" value="1" name="actual_opt[]" >Not Ok';
	 }
	 else{
		$text_req='<p style="display:none;"><input type="checkbox" value="0" name="actual_opt[]" class="check_req."'.$sr.'"" onclick="return false" checked> No</p>';
 
	 }*/
	

     $query_remark1 ="select remarks,actual_output from tbl_transactions where stage_id='1' and station_id='$station_id' and product_id='$product_id'";
   ...
   $query_remark = odbc_exec($conn, $query_remark1);
	  ...
	  $actual_otp ='<input type="radio" class="check_req" value="1" name="actual_opt'.$sr.'" id="check_req'.$sr.'" '.$check_o.' > Ok
	  <input type="radio" class="check_req" value="0" name="actual_opt'.$sr.'" id="check_req'.$sr.'" '.$check_z.'> Not Ok';
