...
SharedManager::saveLog("log_manual_details", "View for Project: $project");
$menu_header_display = 'Manual Management';
...
<html>
<head>
    <title>OneX | Manual Management</title>
    ...
    <style>
        ...
        * {
        ...
        /* ✅ ACTION BUTTONS - STYLED LIKE document_details.php */
        .action-buttons {
            display: flex;
            gap: 12px;
            align-items: center;
...
<body>
<div id="wrapper">
    <?php $activePage = '/manual_details.php'; ?>
                ...
                <div class="filter-section">
                    <div class="filter-row">
                        <div class="filter-group">
                            <label for="filterManualType">Manual Type</label>
                            <select id="filterManualType" class="form-control filter-select">
    ...
    <?php $footer_display = 'Manual Management';
    include_once '../../assemblynotes/shared/footer.php'; ?>
</div>

<!-- ✅ EDIT MODAL - WITH CORRECTED VERSION LOGIC -->
<div id="editModalOverlay" class="edit-modal-overlay">
    <div id="editModal" class="edit-modal">
        <div class="edit-modal-header">
            <h2 class="edit-modal-title">Edit Manual</h2>
        ...
        <div class="edit-modal-body">
            <form id="editManualForm">
                ...
                <input type="hidden" id="hasExistingFile" value="">
                <input type="hidden" id="currentVersion" value="">

                <!-- Row 1: Manual Type, Version, Issue Date -->
                <div class="edit-form-row">
                    <div class="edit-form-group">
                        <label for="editManualType">Manual Type <span class="required">*</span></label>
                        <select id="editManualType" class="edit-form-select" required>
                            <option value="">-- Select Manual Type --</option>
                        </select>
                    </div>

                    <div class="edit-form-group">
                        <label for="editVersion">Current Version</label>
                        <input type="text" id="editVersion" class="edit-form-input" placeholder="0.0" disabled>
                ...
                <div class="edit-form-row full">
                    <div class="edit-form-group">
                        <label>Upload Document (PDF) <span class="required">*</span></label>
                        <div class="edit-upload-area" id="uploadArea">
                            <i class="fa fa-cloud-upload"></i>
                            <p>Click to upload or drag and drop</p>
                            <small>Supported format: PDF (.pdf) - Max 50MB</small>
                        </div>
...
<script>
    ...
    // ✅ CORRECTED: Increment major version (0.0 → 1.0 → 2.0, etc.)
    function incrementVersionMajor(currentVersion) {
    ...
    function calculateNextVersion(currentVersion, fileSelected) {
        if (!fileSelected) {
            // No file selected, show current version
        ...
        if (!currentVersion || currentVersion === '0.0') {
            ...
            // Increment version
            return incrementVersionMajor(currentVersion);
        }
    }

    // ✅ UPDATE VERSION INFO DISPLAY
    function updateVersionInfo() {
        const currentVersion = $('#currentVersion').val();
        const hasFile = selectedFile !== null;
        const nextVersion = calculateNextVersion(currentVersion, hasFile);
        
        const versionInfoDiv = document.getElementById('versionInfo');
        
        if (hasFile) {
            if (currentVersion === '0.0' || !currentVersion) {
                versionInfoDiv.innerHTML = `<strong>New Upload:</strong> Will be saved as v${nextVersion}`;
                versionInfoDiv.style.color = '#28a745';
            } else {
                versionInfoDiv.innerHTML = `<strong>Update:</strong> Will increment to v${nextVersion}`;
                versionInfoDiv.style.color = '#0099ff';
            }
        } else {
            versionInfoDiv.innerHTML = `<strong>Current:</strong> v${currentVersion || '0.0'} (No file selected)`;
            versionInfoDiv.style.color = '#999';
        }
    }
    ...
    function handleFileSelect(file) {
        ...
        // ✅ Update version info when file is selected
        updateVersionInfo();
    }

    // ✅ OPEN EDIT MODAL - CORRECTED VERSION LOGIC
    function openEditModal(manualId) {
        showFullLoader();
        selectedFile = null;
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    // Populate form
                    $('#editManualId').val(manual.id);
                    $('#editManualType').val(manual.manual_type);
                    
                    // ✅ CORRECTED: Check if file exists
                    const hasFile = manual.file_path && manual.actual_file_name;
                    $('#hasExistingFile').val(hasFile ? '1' : '0');
                    
                    // ✅ Display current version
                    const currentVersion = manual.version || '0.0';
                    $('#editVersion').val(currentVersion);
                    ...
                    // ✅ Set Issue Date with DD/MM/YYYY format
                    const issueDateFormatted = formatDateToDDMMYYYY(manual.issue_date);
                    $('#editIssueDate').val(issueDateFormatted);
                    
                    $('#editRemark').val(manual.remark || '');
                    ...
                    // ✅ INITIALIZE DATE PICKER AFTER MODAL SHOWS
                    setTimeout(() => {
                        initializeEditIssueDatePicker();
                    }, 500);
                } else {
                    toastr.error(response.message || 'Failed to load manual');
                }
            },
            error: function(xhr, status, error) {
                hideFullLoader();
                console.error('Error:', error);
                toastr.error('Error loading manual');
            }
        });
    }

    // ✅ INITIALIZE DATE PICKER FOR ISSUE DATE
    ...
    // ✅ SAVE MANUAL CHANGES - CORRECTED VERSION LOGIC
    function saveManualChanges() {
        document.getElementById('editDocumentFileError').textContent = '';
        document.getElementById('editIssueDateError').textContent = '';
        
        const manualId = $('#editManualId').val();
        const manualType = $('#editManualType').val();
        ...
        if (!manualType) {
            isValid = false;
            toastr.error('Please select a manual type');
        }
        ...
        // ✅ CORRECTED: Calculate new version
        let newVersion = calculateNextVersion(currentVersion, true);
        ...
        formData.append('action', 'updateManual');
        formData.append('id', manualId);
        formData.append('manual_type', manualType);
        formData.append('manual_version', newVersion); // ✅ Send version
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    toastr.success('Manual updated successfully! Version: ' + newVersion);
                    closeEditModal();
                    dataTable.ajax.reload();
                } else {
                    toastr.error(response.message || 'Failed to update manual');
                }
            },
            error: function(xhr, status, error) {
                hideFullLoader();
                ...
                toastr.error('Error updating manual');
            }
        });
    }

    // ✅ OPEN DOCUMENT VIEWER - FIXED FIELD NAME
    function openDocumentViewer(manualId, fileName) {
        if (!manualId || !fileName) {
            toastr.error('Manual information is missing');
            return;
        }
        ...
        if (fileExtension !== 'pdf') {
            toastr.error('Only PDF files are supported');
            return;
        }
        ...
        viewerState.manualId = manualId;
        viewerState.pageCache = {};
        viewerState.applyWatermark = !userPermissions.hasAdminRights;
        ...
        if (viewerState.applyWatermark) {
            watermarkIndicator.style.display = 'inline-block';
        } else {
        ...
        const formData = new FormData();
        formData.append('action', 'viewManualFile');
    ...
    function renderPageSequentially(pageNum, container) {
        ...
        viewerState.pdfDoc.getPage(pageNum).then(page => {
            ...
            page.render(renderContext).promise.then(() => {
                ...
                // Apply watermark only for non-admin users
                if (viewerState.applyWatermark) {
                    const watermarkOverlay = document.createElement('div');
                    watermarkOverlay.className = 'document-watermark-container';
                    watermarkOverlay.innerHTML = '<div class="document-watermark-text">CONTROLLED COPY</div>';
                    pageContainer.appendChild(watermarkOverlay);
                }
    ...
    function downloadDocument() {
        if (!viewerState.manualId || !viewerState.fileName) {
            toastr.error('Manual information is not available');
            return;
        }
        ...
        const formData = new FormData();
        formData.append('action', 'downloadManualPdf');
        formData.append('manual_id', viewerState.manualId);
        formData.append('apply_watermark', viewerState.applyWatermark ? '1' : '0');
    ...
    function loadFilterOptions() {
        ...
        $.ajax({
            ...
            type: 'POST',
            dataType: 'json',
            data: { action: 'getManualTypes' },
            success: function(response) {
                const select = $('#filterManualType');
                const editSelect = $('#editManualType');
            ...
            error: function(xhr, status, error) {
                console.error('Error loading manual types:', error);
                toastr.error('Error loading manual types');
            }
        });
    }
    ...
    function fetchAllFilteredData() {
        return $.ajax({
            ...
            dataType: 'json',
            data: {
                action: 'getAllManuals',
                manual_type: currentFilters.manual_type,
    ...
    function exportToExcel() {
        ...
        fetchAllFilteredData().done(function(response) {
            ...
            const worksheet = workbook.addWorksheet('Manuals');

            const headers = ['Manual Type', 'Version', 'Issue Date', 'Remark'];
            ...
            workbook.xlsx.writeBuffer().then(buffer => {
                ...
                const a = document.createElement('a');
                a.href = url;
                a.download = 'Manual_' + moment().format('YYYY-MM-DD_HH-mm-ss') + '.xlsx';
                a.click();
                window.URL.revokeObjectURL(url);
                hideFullLoader();
    ...
    function exportToPDF() {
        ...
        fetchAllFilteredData().done(function(response) {
            ...
            doc.text('Manual Report', margin, margin + 3);
            ...
            doc.save('Manual_' + moment().format('YYYY-MM-DD_HH-mm-ss') + '.pdf');
            hideFullLoader();
    ...
    $(document).ready(function() {
        ...
        document.getElementById('editModalSaveBtn').addEventListener('click', saveManualChanges);
        ...
        dataTable = $('#resultsTable').DataTable({
            ...
            ajax: {
                ...
                data: function(d) {
                    d.action = 'getManuals';
                    d.manual_type = currentFilters.manual_type;
                ...
                },
                {
                    ...
                    render: function(data) {
                        ...
                        if (userPermissions.hasEditRights) {
                            actionHtml += `<button class="action-btn" onclick="openEditModal(${data.id})" title="Edit Manual">
                                                <i class="fa fa-edit"></i>
                                            </button>`;
                        }

                        // ✅ FIXED: Check for actual_file_name instead of file_name
                        if (userPermissions.hasViewRights && data.file_path && data.actual_file_name) {
                            actionHtml += `<button class="action-btn" onclick="viewManual(${data.id})" title="View Manual">
        ...
        $('#applyFiltersBtn').click(function() {
            ...
            currentFilters = {
                manual_type: escapeSpecialChars($('#filterManualType').val().trim()),
        ...
        // ✅ VIEW MANUAL FUNCTION - FIXED FIELD NAME
        window.viewManual = function(id) {
            showFullLoader();
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        // ✅ FIXED: Use actual_file_name instead of file_name
                        if (manual.file_path && manual.actual_file_name) {
                            openDocumentViewer(id, manual.actual_file_name);
                        } else {
                            toastr.error('No file available for this manual');
                        }
                    } else {
                        toastr.error(response.message || 'Failed to load manual');
                    }
