...
$query =  DbManager::fetchPDOQueryData('spectra_db',$sql,[":stage_type" => "Subassembly",":stage_id" => "$stage_id"])["data"];

//To get rating and client name
...
<div class="wrapper">
 ...
 <!-- <a href="barcodegenerate.php"  class="tablinks" ><i class="fas fa-home"></i> </a>
      ...
      <div class="modal-content">
       ...
       <label>PDF UPLOAD: </label>
		 ...
		 <div id="div_defect" style="display:none">
           ...
           <label>Safety: </label>
       <select name="Safety_type" class="select2 form-control" id="Safety_type">
				  ...
				  <textarea class="form-control" name="safety_rem" id="safety_rem"></textarea>                   
                  <!-- <label>Start Date: </label>
                                 <input type="text" class="form-control" name="start_date_safety" id="start_date_safety" value ="<?php echo date('Y-m-d h:i:s'); ?>" readonly>
                                 <label>End Date: </label>
                                 <input type="text" class="form-control" name="end_date_safety" id="end_date_safety" readonly> -->
       ...
       <select name="missing_type" class="select2 form-control" id="missing_type">
				  ...
				  <textarea class="form-control" name="missing_rem" id="missing_rem"></textarea> 				  
       ...
       <select name="infra_type" class="select2 form-control" id="infra_type">
				  ...
				  <textarea class="form-control" name="Infra_remark" id="Infra_remark"></textarea> 
...
if($i == 1){
echo '<div style="display: flex; flex-direction: column; justify-content: center; align-items: center;"><h7>'.strtoupper($query1['stage_name']).'</h7><button type="button" id="div_check'.$i.'"  title="'.strtoupper($query1['stage_name']).'" stage-id="'.$query1['stage_id'].'" data-id="'.$query1['stage_name'].'" data-toggle="modal" data-target="#myModal1" class="btn btn-success btn-circle btn-xl_pre stage_clk">'.$arr[$j].'
                            </button>
</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
}
if($i == 2){
    echo '<div style="display: flex; flex-direction: column; justify-content: center; align-items: center;"><h7>'.strtoupper(substr($query1['stage_name'],0,14)).'..</h7><button type="button" id="div_check'.$i.'"  title="'.strtoupper($query1['stage_name']).'" class="btn btn-default btn-circle btn-xl">'.$arr[$j].'
    </button>
</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
}else if($i != 1 && $i != 2 )
{
echo '<div style="display: flex; flex-direction: column; justify-content: center; align-items: center;"><h7>'.strtoupper($query1['stage_name']).'</h7><button type="button" id="div_check'.$i.'"  title="'.strtoupper($query1['stage_name']).'" class="btn btn-white btn-circle btn-xl">'.$arr[$j].'
                            </button>
</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
}
$j++; $i++; } ?>
...
'language': {
    ...
    "ajax": {
        'type': 'POST',
        'url': 'load_subassemblystage2.php',
        'data': function (data) {
 ...
 // count--;
  return '<div style="display:none;"><input type="radio" class="check_req" value="3" name="actual_opt'+row['sr_no']+'" id="check_req'+row['sr_no']+'" checked ></div>';
                    ...
                    {mData: 'remark',sWidth: '40%'},
                    {mData: 'sr_no',sWidth: '10%',
                      render: function (mdata, type, row, meta){
...
else if(textfill == "NotFill")
{
 ...
 if (confirm('Please confirm: Do you want to complete the second stage?')) {
       $.ajax({
           url: 'save_secondstage.php',
           type: "POST",
           data: {
...
$.ajax({
  ...
  var end_date = $('#end_date_Infra').val();
...
if (confirm('Are you sure you want to Update first stage details?')) {
  $.ajax({
