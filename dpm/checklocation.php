...
$scanqrcode = $_SESSION['scanqrcode'];
  ...
  $sql = "SELECT * FROM tbl_stage where stage_type=:Subassembly";
	
  $query = DbManager::fetchPDOQueryData('spectra_db',$sql,[":Subassembly" => "Subassembly"])["data"];
      ...
      <div class="modal-content">
        ...
        <div class="modal-footer">
			...
			<div class="card">
              ...
              <input type="hidden" id="checkstage_id" value="3"> 
             <div class="row">
                <div class="col-md-3">
                    <label>Scan Barcode 1 :</label>
                </div>
                <div class="col-md-8">
                    <input type="text" id="scan_barcode1" name="scan_barcode1" class="form-control" autofocus>
                </div>

             </div>
             <div class="row">
                <div class="col-md-3">
                    <label>Scan Barcode 2 :</label>
                </div>
                <div class="col-md-8">
                    <input type="text" id="scan_barcode2" name="scan_barcode2" class="form-control">
                </div>

             </div>
             <div class="row">
                <div class="col-md-3">
                    <label>Scan Barcode 3 :</label>
                </div>
                <div class="col-md-8">
                    <input type="text" id="scan_barcode3" name="scan_barcode3" class="form-control">
                </div>
...
<div class="row">
			...
			<div class="card" style="height: 292px; display: flex; flex-direction: column; justify-content: space-between; padding: 10px; overflow: hidden;">
...
{
echo '<div style="display: flex; flex-direction: column; justify-content: center; align-items: center;"><h7>'.strtoupper($query1['stage_name']).'</h7><button type="button" id="div_check'.$i.'"  title="'.strtoupper($query1['stage_name']).'" class="btn btn-default btn-circle btn-xl">'.$arr[$j].'
                            </button>
</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
}
$j++; $i++; 
...
'language': {
	  ...
	  'destroy': true,
	    "sAjaxSource": "load_assemblystage2.php",
         "aoColumns": [
                    {mData: 'sr_no'},
					{mData: 'check_item'},
					{mData: 'actual_opt'},
...
var scan_barcode1 =$('#scan_barcode1').val();
var scan_barcode2 =$('#scan_barcode2').val();
var scan_barcode3 =$('#scan_barcode3').val();
...
if(scan_barcode1 == ""){
     alert("Please scan at least one barcode to finish");
     return false;

}
else {
  if (confirm('Please confirm: Do you want to complete the last stage?')) {
        $.ajax({
            url: 'save_locationstage.php',
            type: "POST",
            data: {
            ...
            success: function (data) {
                ...
                var x = data.success; 
               if(x.trans == "SAVED" && x.barcode1 == "SAVED" && x.barcode2 == "SAVED" && x.barcode3 == "SAVED" && x.saved=="SAVED" && x.complete == "COMPLETE")
                   {
                    window.location.href = 'barcodegenerate.php'; 
                    alert("Data saved successfully!");
                   }
				   else if(x.trans == "SAVED" &&( x.barcode1 == "SAVED" || x.barcode2 == "SAVED" || x.barcode3 == "SAVED" ) && x.saved=="SAVED" && x.complete == "COMPLETE")
                   {
                    window.location.href = 'barcodegenerate.php'; 
                    alert("Data saved successfully!");
                   }
				   else if(x.trans == "SAVED" && (x.barcode1 == "SAVED"  || x.barcode2 == "SAVED" || x.barcode3 == "SAVED") && x.saved=="SAVED" && x.notcomplete == "NOTCOMPLETE")
                   {
                    window.location.href = 'barcodegenerate.php'; 
                    alert("Data saved successfully!");
                   }
...
$.ajax({
 ...
 else if(work_id == '4' || work_id == '6' || work_id == '7' || work_id == '8' || work_id == '9' || work_id == '11' || work_id == '12' )
 {
   if(work_id == '4'){
...
if(tran_stage_id == '1')
{
  var stage_desc="First";
}
else if(tran_stage_id == '2')
{
  var stage_desc="Second";
}
...
if (confirm('Are you sure you want to Update '+stage_desc+' Subassembly stage details?')) {
$.ajax({
