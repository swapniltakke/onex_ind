...
if (!$query) {
  echo "ERROR"; 
	...
	$resuploadid = DbManager::fetchPDOQuery('spectra_db', $sql2, [":ProgressStatus" => "3", ":scanqrcode" => "$scanqrcode", ":progress_status" => "4"]);
  $res['saved'] = "SAVED";
  ...
  $result = DbManager::fetchPDOQueryData('spectra_db', $check_nextstage, [":product_id" => "%,$product_id,%", ":stage_id" => "%,$stage_id,%"])["data"];
  $cnt = $result[0]['total'];

  $trans_check = "select count(*) as cnttotal from tbl_transactions where product_id =:product_id and stage_id=:stage_id";
  $rs = DbManager::fetchPDOQueryData('spectra_db', $trans_check, [":product_id" => "$product_id", ":stage_id" => "$stage_id"])["data"];
