...
$query = DbManager::fetchPDOQueryData('spectra_db',$sql,[":stage_type" => "Testing",":stage_id" => "$stage_id"])["data"];
...
<div class="row">
         ...
         <?php $i=1; foreach ($querywork as $querywork1) { ?>
          <div class="col-md-4"><a style="cursor: pointer;" data-id="<?php echo $querywork1['id']; ?>" work-id="<?php echo $querywork1['title']; ?>" data-toggle="modal" class="workbench_clk" data-target="#myModal"><?php echo html_entity_decode($querywork1['title_icon'])."<br>".html_entity_decode($querywork1['title']); ?></a>
        </div>
         <?php $i++;  } ?>
         </div>
            <br>
...
if($i == 1){
echo '<div><h7>'.strtoupper($query1['stage_name']).'</h7><br><button type="button" id="div_check'.$i.'"  title="" class="btn btn-default btn-circle btn-xl">'.$arr[$j].'
                            </button>
</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
} else 
{
echo '<div><h7>'.strtoupper($query1['stage_name']).'</h7><br><button type="button" id="div_check'.$i.'"  title="" class="btn btn-white btn-circle btn-xl">'.$arr[$j].'
                            </button>
</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
}
$j++; $i++; } ?>
...
'language': {
    ...
    "ajax": {
        'type': 'POST',
        'url': 'load_testingstage1view.php',
        'data': function (data) {
...
if(remarkcnt.length == '') 
{
  ...
  if (confirm('Are you sure you want to complete first stage of testing?')) {
        $.ajax({
            url: 'Update_firststagetesting.php',
            type: "POST",
            data: {
