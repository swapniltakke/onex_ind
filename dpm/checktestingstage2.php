...
else if(textfill == "NotFill")
{
  ...
  if (confirm('Please confirm: Do you want to complete the last stage of testing?')) {
        $.ajax({
            url: 'save_secondstagetesting.php',
            type: "POST",
            data: {
            ...
            success: function (data) {
				...
				var x = data.success; 
				//echo x.saved ."  " response;
                if(x.saved == "SAVED")
                    {
                        alert("Data saved successfully!");
                       //  window.location.href = 'http://localhost/Spectra/checktestingstge2.php';   
...
$.ajax({
  ...
  var Missing_remark = $('#Missing_remark').val();
  if(work_type == "11"){
...
$.ajax({
  ...
  var end_date = $('#end_date_Missing').val();
  var Infra_remark = $('#Infra_remark').val();
  if(work_type == "11"){
